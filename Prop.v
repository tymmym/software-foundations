...
Proof.
  ...
  Case "m = 0". apply b_0.
  Case "m = S m'". simpl. apply b_sum. apply B. apply IHm'.  Qed.
(** [] *)
...
Proof.
  intros n G. apply g_plus3. apply g_plus5. apply g_plus5. apply G.  Qed.
(** [] *)
...
Proof.
  intros n m G1 G2. induction G1 as [| n'| n'].
  Case "g_0". simpl. apply G2.
  Case "g_plus3". apply g_plus3. apply IHG1.
  Case "g_plus5". apply g_plus5. apply IHG1.  Qed.
(** [] *)
...
Proof.
  intros x y z. rewrite -> plus_swap. apply plus_assoc.  Qed.

Theorem g_times2: forall n, gorgeous n -> gorgeous (2*n).
Proof.
  intros n H. simpl. destruct H.
  Case "g_0". apply g_0.
  Case "g_plus3".
    assert (Hplus3: gorgeous (3 + n)).
      apply g_plus3. apply H.
    apply gorgeous_sum.
      apply Hplus3.
      rewrite -> plus_0_r. apply Hplus3.
  Case "g_plus5".
    assert (Hplus5: gorgeous (5 + n)).
      apply g_plus5. apply H.
    apply gorgeous_sum.
      apply Hplus5.
      rewrite -> plus_0_r. apply Hplus5.  Qed.
(** [] *)
...
Theorem ev_sum : forall n m,
   ev n -> ev m -> ev (n+m).
Proof.
  intros n m E1 E2. induction E1 as [| n'].
  Case "E1 = ev_0". apply E2.
  Case "E2 = ev_SS". apply ev_SS. apply IHE1.  Qed.
(** [] *)
...
Theorem SSSSev__even : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof.
  intros n E. inversion E. inversion H0. apply H2.  Qed.
...
Proof.
  ...
  Case "H2 = ev_SS".
    apply IHev. inversion H1. apply H0.  Qed.
(** [] *)
...
Proof.
  ...
  assert (H: n + n + (m + p) = n + m + (n + p)).
    ...
    assert (Hswap: n + (m + p) = m + (n + p)).
      apply plus_swap.
    rewrite -> Hswap. apply plus_assoc.
  rewrite H. apply ev_sum. apply E1. apply E2.
  rewrite <- double_plus. apply double_even.  Qed.
(** [] *)
...
Theorem pal_append : forall (X : Type) (l : list X),
  pal (l ++ rev l).
Proof.
...
Proof.
  ...
  Case "P = p_one".
    reflexivity.
  Case "P = p_cons".
    simpl. rewrite -> rev_snoc. simpl. rewrite <- IHP. reflexivity.  Qed.
(** [] *)
...
Theorem pal_converse : forall (X : Type) (l : list X),
  l = rev l -> pal l.
Proof.
  intros X l. induction l as [| x xs].
  Case "l = nil".
    intros eq. apply p_nil.
  Case "l = cons".
    rewrite <- rev_involutive with (l:=xs).
    destruct (rev xs) as [| x' xs'].
    SCase "rev xs = nil".
      intros eq. apply p_one.
    SCase "rev xs = cons".
      simpl. rewrite rev_snoc. simpl.
      intros eq. inversion eq. apply p_cons.
      (* I'm stuck - can't build usefull inductive hypothesis *)
Admitted.
(** [] *)
...
Proof.
  ...
  Case "H = s_nil".
    apply s_nil.
  Case "H = s_true".
    apply s_true. apply IHsubseq.
  Case "H = s_false".
    apply s_false. apply IHsubseq.  Qed.
